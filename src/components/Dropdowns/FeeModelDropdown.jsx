...
function FeeModelDropdown({
  ...
  const fetchFeeSteps = async () => {
    try {
      const res = await fetch(`http://127.0.0.1:5000/hiring_steps/fee_model/${applicantId}`);
  ...
  const getStatusBadge = completed => {
    return completed ? <Badge className="bg-green-50 text-green-700 border-green-200">
        Completed
  ...
  return <div className="space-y-4 p-4 bg-card rounded-lg border border-border">
      ...
      {feeSteps.map(step => <div key={step.id} className="border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="flex gap-2 pt-2">
            <Button size="sm" variant="default" onClick={e => handleStepAction(e, step.title, "finish")}>
              Finish Step
            </Button>
            <Button size="sm" variant="outline" onClick={e => handleStepAction(e, step.title, "skip")}>
              Skip Step
            </Button>
          </div>
        </div>)}
